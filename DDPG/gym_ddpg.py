...
class GymDDPG:
    ...
    def train(self):
        ...
        while self.episode < self.episodes:
            ...
            while not done:
                for i in range(len(self.agents)):
                    action = self.agents[i].action(states[i])
            ...
            self.agents[0].critic_network.is_episode_done = True
            ...
            for i in range(len(self.agents)):
                if self.episode > 0 and self.episode % 50 == 0:
                    #loss_x=np.arange(len(self.agents[0].critic_network.losses))
                    loss_y=self.agents[0].critic_network.losses
                    # ldf=pd.Series(loss_y)
                    # lfwd = ewma(ldf,span=10).mean()
                    plt.plot(loss_y,'r')
                    plt.ylabel("loss")
                    plt.xlabel("episode")
                    plt.savefig("logs/loss"+str(self.episode)+".png")
                    plt.clf()
                    clear_output(wait=False)
                    x_axis=np.arange(self.episode+1)
                    plt.plot(x_axis,rewards2print,'b')
                    df=pd.Series(rewards2print)
                    fwd = ewma(df,span=10).mean()
                    plt.plot(fwd,'r')
                    plt.ylabel("reward")
                    plt.xlabel("episode")
